...
class PurchaseActivity : BaseSimpleActivity() {
    ...
    private lateinit var hmsHelper: HmsHelper
...
//        hmsHelper.retrieveDonation(productIdList, subscriptionIdListAll)
        ...
        hmsHelper.subSkuDetailsInitializedLiveData.observe(this) {
        ...
        hmsHelper.isIapPurchasedListLiveData.observe(this) {
...
//                    hmsHelper.retrieveDonation(productIdList, subscriptionIdListAll)
                ...
                R.id.openSubscriptions -> {
                    val url = "https://appgallery.huawei.com"
    ...
    override fun onActivityResult(requestCode: Int, resultCode: Int, data: Intent?) {
        ...
        // Processing the purchase result HMS
        hmsHelper.handlePurchaseResult(requestCode, resultCode, data)
    }
    ...
    private fun setupButtonIapPurchased() {
        binding.appOneButton.apply {
            val price = hmsHelper.getPriceDonation(productIdList[0])
            isEnabled = price != getString(stringsR.string.no_connection)
            text = price
            setOnClickListener {
                hmsHelper.getDonation(productIdList[0])
            }
            background.setTint(primaryColor)
        }

        binding.appTwoButton.apply {
            val price = hmsHelper.getPriceDonation(productIdList[1])
            isEnabled = price != getString(stringsR.string.no_connection)
            text = price
            setOnClickListener {
                hmsHelper.getDonation(productIdList[1])
            }
            background.setTint(primaryColor)
        }

        binding.appThreeButton.apply {
            val price = hmsHelper.getPriceDonation(productIdList[2])
            isEnabled = price != getString(stringsR.string.no_connection)
            text = price
            setOnClickListener {
                hmsHelper.getDonation(productIdList[2])
            }
            background.setTint(primaryColor)
        }
    }
    ...
    private fun setupButtonSupPurchased() {
        ...
        binding.appTwoSubButton.apply {
            val price = hmsHelper.getPriceSubscription(subscriptionIdList[1])
            if (price != getString(stringsR.string.no_connection)) {
                isEnabled = true
                val textPrice = String.format(getString(stringsR.string.per_month), price)
                text = textPrice
                setOnClickListener {
                    hmsHelper.getSubscription(subscriptionIdList[1])
                }
            } else {
                text = price
            }
        ...
        binding.appThreeSubButton.apply {
            val price = hmsHelper.getPriceSubscription(subscriptionIdList[2])
            if (price != getString(stringsR.string.no_connection)) {
                isEnabled = true
                val textPrice = String.format(getString(stringsR.string.per_month), price)
                text = textPrice
                setOnClickListener {
                    hmsHelper.getSubscription(subscriptionIdList[2])
                }
            } else {
                text = price
            }
        ...
        binding.appOneSubYearButton.apply {
            val price = hmsHelper.getPriceSubscription(subscriptionYearIdList[0])
            if (price != getString(stringsR.string.no_connection)) {
                isEnabled = true
                val textPrice = String.format(getString(stringsR.string.per_year), price)
                text = textPrice
                setOnClickListener {
                    hmsHelper.getSubscription(subscriptionYearIdList[0])
                }
            } else {
                text = price
            }
        ...
        binding.appTwoSubYearButton.apply {
            val price = hmsHelper.getPriceSubscription(subscriptionYearIdList[1])
            if (price != getString(stringsR.string.no_connection)) {
                isEnabled = true
                val textPrice = String.format(getString(stringsR.string.per_year), price)
                text = textPrice
                setOnClickListener {
                    hmsHelper.getSubscription(subscriptionYearIdList[1])
                }
            } else {
                text = price
            }
        ...
        binding.appThreeSubYearButton.apply {
            val price = hmsHelper.getPriceSubscription(subscriptionYearIdList[2])
            if (price != getString(stringsR.string.no_connection)) {
                isEnabled = true
                val textPrice = String.format(getString(stringsR.string.per_year), price)
                text = textPrice
                setOnClickListener {
                    hmsHelper.getSubscription(subscriptionYearIdList[2])
                }
            } else {
                text = price
            }
    ...
    private fun setupButtonSupChecked() {
        val check = AppCompatResources.getDrawable(this@PurchaseActivity, R.drawable.ic_check_circle_mini)
        if (hmsHelper.isSubPurchased(subscriptionIdList[0])) {
        ...
        if (hmsHelper.isSubPurchased(subscriptionIdList[1])) {
        ...
        if (hmsHelper.isSubPurchased(subscriptionIdList[2])) {
        ...
        if (hmsHelper.isSubPurchased(subscriptionYearIdList[0])) {
        ...
        if (hmsHelper.isSubPurchased(subscriptionYearIdList[1])) {
        ...
        if (hmsHelper.isSubPurchased(subscriptionYearIdList[2])) {
